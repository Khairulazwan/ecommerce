<?php // Simple Ajax Chat > JavaScript
	...
	$alert_title = __('New chat message', 'simple-ajax-chat');
...
function insertNewContent(liName,liText,lastResponse, liUrl, liId) {
	...
	if ('Notification' in window) {
		sacCustomNotify('<?php echo $alert_title; ?>', liName + ': ' + sac_apply_filters(liText) + "\r\n" + '(' + datetime + ')', window.location.href);
	}
...
// browser alerts @ https://m0n.co/10
function sacCustomNotify(title, desc, url) {
	if (Notification.permission !== 'granted') {
		Notification.requestPermission();
	} else {
		var notification = new Notification(title, {
			icon:'<?php echo plugins_url(basename(dirname(dirname(__FILE__)))) .'/resources/sac-400.png'; ?>',
			body: desc,
		});
		if (url) {
